...
function getRequest(user) {
  const url = `${user}/user/notification/coming/filter`;
  var options = {
    'headers': {
...
exports.handler = async (event) => {
    ...
    try {
      ...
      if (Object.keys(result).length === 0) {
          response["dialogAction"]['message']["content"] = "You don't have any coming notifications!";
      } else {
          response["dialogAction"]['message']["content"] = 'Their are notifications, this feature is coming soon!'
      }
    } catch (error) {
